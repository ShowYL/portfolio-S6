    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {realisations.map((project, index) => (
          <div
            key={index}
            className="group bg-white/40 dark:bg-slate-900/40 backdrop-blur-md rounded-2xl border border-white/20 dark:border-white/10 flex flex-col shadow-sm hover:shadow-lg transition-all duration-300"
          >
            <div className="p-6 flex-grow">
              <div className="flex justify-between items-start mb-4">
                <div className="p-3 bg-blue-50/50 dark:bg-blue-900/20 rounded-xl text-blue-600 dark:text-blue-400">
                  <project.icon className="w-8 h-8" />
                </div>
                <span className="px-3 py-1 bg-white/50 dark:bg-slate-800/50 text-[10px] font-bold rounded-full text-slate-600 dark:text-slate-300 border border-white/20">
                  {project.category}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                {project.tags.map((tag, idx) => (
                  <span
                    key={idx}
                    className="text-[10px] uppercase tracking-wider font-bold text-slate-500 dark:text-slate-400 bg-white/30 dark:bg-slate-800/30 border border-white/10 dark:border-white/5 px-2 py-0.5 rounded"
                  >
                    {tag}
                  </span>
                ))}
            ...
            {(project.github || project.link) && (
              <div className="px-6 py-4 bg-white/30 dark:bg-white/5 border-t border-white/20 dark:border-white/10 rounded-b-2xl flex items-center gap-4">
                {project.github && (
                  <a
                    href={project.github}
                    target="_blank"
